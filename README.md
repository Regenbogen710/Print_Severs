...
调度器默认使用 FIFO：先上传的 `waiting` 任务排在前面并优先打印。网页队列表中等待任务左侧有拖动手柄；需要先在打印队列区域输入 `config.ini` 中配置的管理员密码完成认证，认证成功后才能拖动等待任务调整打印先后顺序，并显示页面最下方的暂停、恢复、删除、重试等管理员操作。认证状态只保存在当前页面内存中，刷新页面后需要重新认证。队列表已预留 `priority` 和 `scheduled_at` 字段，后续可扩展优先级调度和定时打印；调度前如果服务已暂停，worker 不会继续处理任务。
